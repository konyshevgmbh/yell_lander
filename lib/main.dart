...
class GameConfig {
  ...
  static const double landerInitialYFactor = 0.4; // Initial Y position relative to screen height (40% from top)
  static const double landerLevelStartY = 50.0; // Absolute Y position at the start of a level
  ...
  static const double surfaceBaseHeightFactor = 0.75; // Base height from top (e.g., 75%)
  static const double surfaceHeightVariationFactor = 0.15; // Variation in height (e.g., +/- 15%)
  static const double surfaceDefaultHeight = 800.0; // Fallback height if terrain points are missing
...
enum GameState { initializing, welcome, playing, levelWonTransition, gameWon, crashed }
...
class _HudOverlayState extends State<HudOverlay> {
  ...
  void initState() {
    ...
    widget.game.onHudUpdate = () => mounted ? setState(() {
      _currentSliderValue = widget.game.minEngineVolumeThreshold;
    }) : null;
  }
  ...
  Widget build(BuildContext context) {
    ...
    switch (game.gameState) {
      ...
      case GameState.crashed:
          displayMessage = GameMessages.crashedOnLevel(game.levelConcluded ?? game.currentLevel);
          break;
    ...
    return Stack(children: [
      ...
      Align(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(GameMessages.fuel(game.fuelValue), style: GameConfig.hudTextStyle),
              Text(GameMessages.vSpeed(game.velocityYValue), style: GameConfig.hudTextStyle),
              Text(GameMessages.level(game.currentLevel), style: GameConfig.hudTextStyle),
            ],
          ),
        ),
      ),
      ...
      // Instructions
      if (game.isRecorderInitialized && game.gameState == GameState.playing)
        Align(
          alignment: Alignment.topRight,
          child: Padding(
            padding: GameConfig.hudPadding,
            child: Text(GameMessages.instructions, style: GameConfig.hudTextStyle),
          ),
        ),
              ...
              children: [
                ...
                Container(
                  ...
                  child: FractionallySizedBox(
                    ...
                    child: Container(color: game.normalizedVolume >= game.minEngineVolumeThreshold 
                      ? GameConfig.hudBarActiveColor : GameConfig.hudBarInactiveColor),
                  ),
                ),
              ],
            ),
          ...
          child: Padding(
            ...
            child: Container(
              ...
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text(GameMessages.threshold(_currentSliderValue), 
                    style: GameConfig.hudSliderTextStyle),
                  SizedBox(
                    width: 150, height: 30,
                    child: Slider(
...
class LunaLanderGame extends FlameGame with TapCallbacks, KeyboardEvents {
  ...
  int currentLevel = 1;
  int? levelConcluded;
  double get currentGravity => GameConfig.baseGravity * math.pow(GameConfig.gravityLevelMultiplier, currentLevel - 1);
  bool _gameComponentsLoaded = false;

  double get fuelValue => _gameComponentsLoaded && lander.isMounted ? lander.fuel : GameConfig.initialFuel;
  double get velocityYValue => _gameComponentsLoaded && lander.isMounted && gameState == GameState.playing ? lander.velocity.y : 0.0;
  ...
  Future<void> onLoad() async {
    ...
    if (!_gameComponentsLoaded) {
      ...
      lander = LunarLander(position: Vector2(size.x / 2, size.y * GameConfig.landerInitialYFactor));
      await add(surface);
      await add(lander);
    ...
    statusMessage = hasPermission && isRecorderInitialized 
        ? GameMessages.welcome
        : (hasPermission ? GameMessages.recorderError : GameMessages.permissionDenied);
    onHudUpdate?.call();
  }
  
  Future<void> _requestPermissionAndInitialize() async {
    bool permGranted = !kIsWeb ? (await Permission.microphone.request()).isGranted : true;
  ...
  void onGameResize(Vector2 canvasSize) {
    ...
    if (_gameComponentsLoaded) {
      final oldWidth = surface.terrainPoints.isNotEmpty ? surface.terrainPoints.last.x : canvasSize.x;
      if (oldWidth > 0) {
  ...
  void update(double dt) {
    ...
    if (isRecorderInitialized) {
      try {
        ...
        if (db != null && db.isFinite) {
          normalizedVolume = ((db.clamp(GameConfig.minDbVolume, GameConfig.maxDbVolume) - GameConfig.minDbVolume) 
                                / GameConfig.dbRange).clamp(0.0, 1.0);
        } else {
    ...
    if (gameState == GameState.playing && isRecorderInitialized && _gameComponentsLoaded) {
      ...
      if (normalizedVolume >= minEngineVolumeThreshold && lander.fuel > 0) {
        double thrustFactor = ((normalizedVolume - minEngineVolumeThreshold) / 
                             (1.0 - minEngineVolumeThreshold).clamp(0.01, 1.0)).clamp(0.0, 1.0);
      ...
      if (lander.position.y >= terrainY - GameConfig.landingDetectionTolerance) {
        ...
        final bool onPad = lander.position.x >= surface.landingPadLeft && 
                         lander.position.x <= surface.landingPadRight;
        final bool safeSpeed = lander.previousVelocityY.abs() <= GameConfig.safeLandingSpeed;
        
        if (onPad && safeSpeed) {
          gameState = currentLevel == GameConfig.maxLevels ? GameState.gameWon 
                                       : GameState.levelWonTransition;
          statusMessage = currentLevel == GameConfig.maxLevels ? "" 
      ...
      lander.position.x = lander.position.x.clamp(halfWidth, size.x - halfWidth);
    } else if (gameState == GameState.welcome && _gameComponentsLoaded) {
      lander.velocity = Vector2.zero();
      if (isRecorderInitialized && normalizedVolume >= minEngineVolumeThreshold) {
        lander.startThrustVisual();
      } else {
        lander.stopThrust();
      }
  ...
  void _processGameAction() {
    if (gameState == GameState.welcome) {
      ...
      currentLevel++;
      _startLevel();
    } else if (gameState == GameState.gameWon || gameState == GameState.crashed) {
      _resetGame();
    }
  }
  ...
  void _resetGame() {
    ...
    if (_gameComponentsLoaded) {
      surface.regenerate(size);
      lander.reset(position: Vector2(size.x / 2, size.y * GameConfig.landerInitialYFactor));  
    }
    gameState = GameState.welcome;
    statusMessage = hasPermission && isRecorderInitialized 
        ? GameMessages.welcome
...
class LunarLander extends PositionComponent {
  ...
  LunarLander({required Vector2 position})
    : super(position: position, size: GameConfig.landerSize, anchor: Anchor.center);
  ...
  void render(Canvas canvas) {
    ...
    if (isExploding) {
      ...
      canvas.drawCircle(Offset.zero, size.x * 1.5, Paint()..color = Colors.red.withOpacity(0.8));
      canvas.drawCircle(Offset.zero, size.x * 0.8, Paint()..color = Colors.orange.withOpacity(0.9));
      return;
    }
    
    // Draw lander body (original drawing)
    canvas.drawRect(
      Rect.fromCenter(center: Offset(0, -size.y * 0.1), width: size.x * 0.8, height: size.y * 0.6),
      Paint()..color = Colors.grey[300]!
    );
    
    // Draw legs (original drawing)
    final legPaint = Paint()..color = Colors.grey[400]!..strokeWidth = 2;
    canvas.drawLine(Offset(-size.x * 0.4, size.y * 0.2), Offset(-size.x * 0.5, size.y / 2), legPaint);
    canvas.drawLine(Offset(size.x * 0.4, size.y * 0.2), Offset(size.x * 0.5, size.y / 2), legPaint);
    ...
    if (_showFlame) {
      final flamePath = Path()
        ...
        ..lineTo(0, size.y * 0.5 + 15 * (0.8 + math.Random().nextDouble() * 0.4))
...
class LunarSurface extends Component {
  ...
  void regenerate(Vector2 gameSize) {
    ...
    landingY = h * (GameConfig.surfaceBaseHeightFactor + (rnd.nextDouble() * GameConfig.surfaceHeightVariationFactor * 2 - GameConfig.surfaceHeightVariationFactor));
    
    terrainPoints = [];
    for (int i = 0; i <= segments; i++) {
      final x = i * segmentWidth;
      final isPadSegment = i >= padStartIndex && i <= padEndIndex;
      final yVariation = GameConfig.surfaceBaseHeightFactor + (rnd.nextDouble() * GameConfig.surfaceHeightVariationFactor * 2 - GameConfig.surfaceHeightVariationFactor);
  ...
  void updateForNewSize(Vector2 newSize) {
    ...
    terrainPoints = terrainPoints.map((p) => Vector2(p.x * scaleX, p.y)).toList();
  ...
  void render(Canvas canvas) {
    ...
    canvas.drawPath(path, Paint()..color = GameConfig.terrainColor..strokeWidth = GameConfig.terrainStrokeWidth..style = PaintingStyle.stroke);
    
    canvas.drawLine(
      Offset(landingPadLeft, landingY),
      Offset(landingPadRight, landingY),
      Paint()..color = GameConfig.landingPadColor..strokeWidth = GameConfig.landingPadStrokeWidth..style = PaintingStyle.stroke
    );
  }
